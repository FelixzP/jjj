const API_URL = process.env.NEXT_PUBLIC_API_URL || 'https://abrasion-bakeshop-oxidation.ngrok-free.dev';
...
export async function apiFetch(path, { method = 'GET', body, token, isMultipart = false } = {}) {
  ...
  if (token) headers['Authorization'] = `Bearer ${token}`; headers['ngrok-skip-browser-warning'] = 'true';
